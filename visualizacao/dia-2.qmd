...
grafico_ordenado <- dados_tri_recente |> 
  mutate(uf_fct = fct_reorder(uf, perc_desocupacao)) |> # <1>
  ggplot() + 
...
dados |>
  ...
  scale_x_date( # <2>
    breaks = "6 months", # <3>
    date_labels = "%m/%Y", # <4>
    minor_breaks = "1 months" # <5>
  ) # <2> 
```

1.  Filtrando os dados para o estado de São Paulo, e considerando apenas os dados a partir de 2019.
...
grafico_customizado <- dados |> # <1>
  ...
  geom_line() + 
  theme_light() + # <2>
  scale_color_viridis_d(end = 0.9) + # <3>
